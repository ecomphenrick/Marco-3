#define _POSIX_C_SOURCE 200809L //para usar o clock
...
static Pixel canvas[VGA_WIDTH][VGA_HEIGHT]; //matriz para armazenar o estado atual do VGA (preto, branco, amarelo, etc)
...
//Syscalls de leitura de arquivo
...
//Usa biblioteca stb_image para carregar PNG e converter para o formato necessário (28x28, 1 canal)
...
void processar_e_salvar_metricas(int* acertos_classe, int* total_classe, int total, int acertos, double media_ms, double desvio_ms, double throughput) {
 double acuracia_geral = (total > 0) ? ((double)acertos / total) * 100.0 : 0.0; //verificação para evitar divisão por zero
...
void exibe_imagem_no_vga(void* endereco, uint8_t* imagem) {
 for (int py = 0; py < 28; py++) { //percorre cada pixel da imagem 28x28
 for (int px = 0; px < 28; px++) {
 uint8_t gray = imagem[py * 28 + px];
 uint8_t cor = gray / 32;
 for (int by = 0; by < 8; by++) { //para cada pixel da imagem, desenha um quadrado 8x8 no VGA
...
void modo_desenho(void* endereco) {
 ...
 printf("-> Clique no Scroll (Meio): Apaga o desenho\n");
 printf("-> Botão Direito: Finaliza e roda a inferência\n");
 ...
 if (middle_click) {
 for (int cy = BOX_Y_START; cy < BOX_Y_END; cy++) {
 for (int cx = BOX_X_START; cx < BOX_X_END; cx++) {
 canvas[cx][cy] = (Pixel){0, 0, 0};
 vga_draw_pixel(endereco, cx, cy, 0, 0, 0);
 }
 }
 }
